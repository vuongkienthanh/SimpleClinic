...
class LineDrugListStateItem:
    ...
    def parse(self) -> "ParsedLineDrugListStateItem":
        return ParsedLineDrugListStateItem(
            **dataclasses.asdict(self)
            | {
                "note": self.parse_note(),
                "sale_unit": self.parse_sale_unit(),
            }
        )
...
class ParsedLineDrugListStateItem:
    ...
    def unparse(self) -> "LineDrugListStateItem":
        return LineDrugListStateItem(
            **dataclasses.asdict(self)
            | {
                "note": self.unparse_note(),
                "sale_unit": self.unparse_sale_unit()
            }
        )


class LineDrugListState:
    def __get__(self, obj: "state.State", objtype=None) -> list[LineDrugListStateItem]:
        return obj._linedrug_list

    def __set__(self, obj: "state.State", _list: list[LineDrugListStateItem]):
